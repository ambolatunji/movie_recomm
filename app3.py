...
# Initialize TMDb API with your API key (replace with your actual TMDb API key)
tmdb.API_KEY = '85b1dd0f04ffa901e6dcca90a7505a21'
...
# Function to display movie details and poster
def display_movie_details(title, link):
    details = get_movie_details(title)
    if details:
        st.warning(f"{details['title']} ({details['release_date'][:4]})")
        st.image(f"https://image.tmdb.org/t/p/w500/{details['poster_path']}", caption=details['overview'], use_column_width=True)
        st.markdown(f"📌 IMDb LINK --- [{details['title']}]({link})")
    else:
        st.warning(title)
...
# If Movie Based Recommendation is being selected this condition will get executed.
if app_options == 'Movie based':
    ...
    if movie_select == '--Select--':
        ...
        for movie, link in table:
            display_movie_details(movie, link)

# If Genre Based Recommendation is being selected this condition will get executed.
